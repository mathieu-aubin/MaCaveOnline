...
<body onresize="center_pagination();">
        ...
        while ($ligne = $res_region->fetch()) {
            if (isset($_POST['affiche_liste'])) {
                if ($ligne['id_region'] == $_POST['select_region']) {
                    ...
                    $select_region .= '<option value='.$ligne['id_region'].'>'.utf8_encode($ligne['region']).'</option>';
                }
        ...
        while ($ligne = $res_type->fetch()) {
            if (isset($_POST['affiche_liste'])) {
                if ($ligne['id_type'] == $_POST['select_type']) {
                    ...
                    $select_type .= '<option value='.$ligne['id_type'].'>'.utf8_encode($ligne['type']).'</option>';
                }
            } elseif (isset($_GET['paged'])) {
        ...
        while ($ligne = $res_stockage->fetch()) {
            if (isset($_POST['affiche_liste'])) {
                if ($ligne['id_lieu_stockage'] == $_POST['select_stockage']) {
                    ...
                    $select_lieu_stockage .= '<option value='.$ligne['id_lieu_stockage'].'>'.utf8_encode($ligne['lieu_stockage']).'</option>';
                }
            } elseif (isset($_GET['paged'])) {
				...
				<?php 
                    ...
                    if (isset($_POST['affiche_liste']) || isset($_GET['paged'])) {
                        ...
                        if (isset($_POST['affiche_liste'])) {
                            ...
                            if (isset($_POST['select_appellation'])) {
                                $appellation = $_POST['select_appellation'];
                            }
                            if (isset($_POST['select_type'])) {
                                $type = $_POST['select_type'];
                            }
                            if (isset($_POST['select_stockage'])) {
                                $stockage = $_POST['select_stockage'];
                            }
                        } elseif (isset($_GET['paged'])) {
                            $region = $_GET['region'];
                        ...
                        if ($region !== '') {
                            $where_region = 'vins.FK_region = '.$region;
                        }
                        //pas d'appellation si Champagne ou Vins étrangers
                        if ($region == 6 || $region == 12) {
                            $where_appellation = 'vins.FK_appellation IS NULL';
                        } elseif ($appellation !== '') {
                            $where_appellation = 'vins.FK_appellation = '.$appellation;
                        }
                        if ($type !== '') {
                            $where_type = 'vins.FK_type = '.$type;
                        }
                        if ($stockage !== '') {
                            $where_stockage = 'utilisateur_vin.FK_lieu_stockage = '.$stockage;
                        }

                        // si pas d'AOC
                        if ($region !== '' && $appellation == '' && $type !== '' && $stockage !== '') {
                            $condition_where = ' AND '.$where_region.' AND '.$where_type.' AND '.$where_stockage;
                        }
                        // si pas de type
                        elseif ($region !== '' && $appellation !== '' && $type == '' && $stockage !== '') {
                            $condition_where = ' AND '.$where_region.' AND '.$where_appellation.' AND '.$where_stockage;
                        }
                        // si pas de stockage
                        elseif ($region !== '' && $appellation !== '' && $type !== '' && $stockage == '') {
                            $condition_where = ' AND '.$where_region.' AND '.$where_appellation.' AND '.$where_type;
                        }
                        // si pas de région (et donc pas d'AOC)
                        elseif ($region == '' && $type !== '' && $stockage !== '') {
                            $condition_where = ' AND '.$where_type.' AND '.$where_stockage;
                        }
                        // si pas de type et pas d'AOC
                        elseif ($region !== '' && $appellation == '' && $type == '' && $stockage !== '') {
                            $condition_where = ' AND '.$where_region.' AND '.$where_stockage;
                        }
                        // si pas de stockage et pas d'AOC
                        elseif ($region !== '' && $appellation == '' && $type !== '' && $stockage == '') {
                            $condition_where = ' AND '.$where_region.' AND '.$where_type;
                        }
                        // si pas de type et pas de stockage
                        elseif ($region !== '' && $appellation !== '' && $type == '' && $stockage == '') {
                            $condition_where = ' AND '.$where_region.' AND '.$where_appellation;
                        }
                        // si pas de région (donc pas d'AOC) et pas de type
                        elseif ($region == '' && $type == '' && $stockage !== '') {
                            $condition_where = ' AND '.$where_stockage;
                        }
                        // si pas de région (donc pas d'AOC) et pas de stockage
                        elseif ($region == '' && $type !== '' && $stockage == '') {
                            $condition_where = ' AND '.$where_type;
                        }
                        // si pas d'AOC et pas de type et pas de stockage
                        elseif ($region !== '' && $appellation == '' && $type == '' && $stockage == '') {
                            $condition_where = ' AND '.$where_region;
                        }
                        // si aucun champ
                        elseif ($region == '' && $appellation == '' && $type == '' && $stockage == '') {
                            $condition_where = '';
                        ...
                        // si tous les champs
                        elseif ($region !== '' && $appellation !== '' && $type !== '' && $stockage !== '') {
                            $condition_where = ' AND '.$where_region.' AND '.$where_appellation.' AND '.$where_type.' AND '.$where_stockage;
                        }
        ...
        if ($total_posts != 0) {
            ...
            switch ($recap) {
                            ...
                            case '0010':
                            {
                                if ($type_de_vin[0] == 'Mousseux') {
                                    $titre_recap = 'Les Mousseux';
                                } else {
                                    $titre_recap = 'Vins '.$type_de_vin[0].'s';
                                }
                            }
                            break;
            ...
            for ($i = 0; $i < count($nom_vin); $i++) {
                ...
                $txt_bouteilles = 'bouteilles';
                            ...
                            if ($nbr_bouteilles[$i] == 1) {
                                $txt_bouteilles = ' bouteille';
                            }
