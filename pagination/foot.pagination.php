<?php
	echo("<div id='navPage'>");
	$options = array('range' => 2, 'posts_per_page' => $posts_per_page); 
	if(isset($region)) $region = "&region=".$region;
	else $region = '';
	if(isset($appellation)) $appellation = "&appellation=".$appellation;
	else $appellation = '';
	if(isset($type)) $type = "&type=".$type;
	else $type = '';
	if(isset($stockage)) $stockage = "&stockage=".$stockage;
	else $stockage = '';
	if(isset($_POST['search'])) $search = "&search=".$_POST['search'];
	else if(isset($_GET['search'])) $search = "&search=".$_GET['search'];
	else $search = '';
