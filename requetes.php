...
/** construction de la fiche du vin sélectionné (fenêtre modale) **/
    if (isset($_POST['id_vin'])) {
        ...
        if ($aoc_fiche !== '' && $aoc_fiche !== NULL) $aoc_fiche .= " - ";
        if ($note_fiche == NULL) $note_fiche = "<em>Non renseigné</em>";
        if ($nb_bouteilles_fiche == NULL) $nb_bouteilles_fiche = "<em>Non renseigné</em>";
        if ($suivi_stock_fiche == 0) $suivi_stock_fiche = "Non";
        else $suivi_stock_fiche = "Oui";
        if ($meilleur_vin_fiche == 0) $meilleur_vin_fiche = "<em>Non renseigné</em>"; 
        else $meilleur_vin_fiche = "Oui";
        if ($prix_achat_fiche == NULL) {
            $prix_achat_fiche = "<em>Non renseigné</em>";
            $prix_achat_edit = "";
        }
        else $prix_achat_edit = $prix_achat_fiche;
        if ($offert_par_fiche == NULL) {
            $offert_par_fiche = "<em>Non renseigné</em>";
            $offert_par_edit = "";
        }
        else $offert_par_edit = $offert_par_fiche;
        if ($lieu_achat_fiche == NULL) $lieu_achat_fiche = "<em>Non renseigné</em>";
        if ($lieu_stockage_fiche == NULL) $lieu_stockage_fiche = "<em>Non renseigné</em>";
        if ($conso_partir_fiche == NULL) $conso_partir_fiche = "<em>Non renseigné</em>";
        if ($conso_avant_fiche == NULL) $conso_avant_fiche = "<em>Non renseigné</em>";
        if ($commentaires_fiche == NULL) $commentaires_fiche = "-";
...
/**insertion des modifications dans la BDD**/
    if (isset($_POST['modifier'])) {
        ...
        if($erreur_offert == '' && $erreur_bt == '' && $erreur_prix == '') {   
            ...
            if($erreur_bt != '') $err_bt = "err_bt=1&";
            else $err_bt = "err_bt=0&";
            if($erreur_offert != '') $err_off = "err_off=1&";
            else $err_off = "err_off=0&";
            if($erreur_prix != '') $err_pri = "err_pri=1&";
            else $err_pri = "err_pri=0&";
...
/**insertion d'un nouveau vin et de sa fiche dans la BDD**/
    if (isset($_POST['ajouter'])) {
        ...
        if($res_verif->rowCount() > 0) $erreur_vin = $vin_exist[0];
        ...
        if ($region == 'autre') {
            ...
            if($res_check->rowCount() > 0) $nom_existant = utf8_decode('Région dejà existante. Veuillez la sélectionner dans la liste.'); 
            /**sinon**/
        ...
        /**si AOC via liste déroulante**/
            ...
            /** si nouvelle AOC**/  
            if ($_POST['select_aoc'] == 'autre') $new_aoc = $_POST['aoc_ajout'];
        ...
        if ($_POST['degre_alcool'] == '') $degre_alcool = NULL;
        else $degre_alcool = $_POST['degre_alcool'];
        //on regarde si on est en mode démo ou non
        $demo = 0;
        if($_SESSION['user'] == 3) $demo = 1;
        /**si pas d'erreur**/
        if ($erreur == '' && $erreur_vin == '') {
            ...
            */
            ...
            //note
            if($_POST['note'] == '') $note = NULL;
            else $note = $_POST['note'];
            //nb_bouteilles
            if($_POST['nb_bouteilles'] == '') $nb_bouteilles = NULL;
            else $nb_bouteilles = $_POST['nb_bouteilles'];
            //vérification de la valeur du champ
            if(!ctype_digit($nb_bouteilles)) $erreur .= "_bouteilles";
            else $erreur .= '_';
            //suivi stock
            if (isset($_POST['suivi_stock'])) $suivi_stock = 1;
            else $suivi_stock = 0;
            ...
            //prix achat
            if($_POST['prix_achat'] == '') $prix_achat = NULL;
            else $prix_achat = $_POST['prix_achat'];
            //vérification de la valeur du champ
            if(!is_numeric($prix_achat) && $prix_achat != NULL) $erreur .= "_prix";
            else $erreur .= '_';
            //offert par
            if($_POST['offert_par'] == '') $offert_par = NULL;
            else $offert_par = $_POST['offert_par'];
            //vérification de la valeur du champ
            if(!is_string($offert_par) && $offert_par != NULL) $erreur .= "_offert";
            else $erreur .= '_';
            //lieu achat
            if($_POST['select_achat'] == '') $lieu_achat = NULL;
            else $lieu_achat = $_POST['select_achat'];
            /**si option autre dans lieu achat
              on récupère la valeur entrée pour l'ajouter à la table lieu_achat**/
            if ($lieu_achat == 'autre') {
                //si on est en mode démo, on empêche l'ajout
            ...
            //lieu stockage
            if($_POST['select_stockage'] == '') $lieu_stockage = NULL;
            else $lieu_stockage = $_POST['select_stockage'];
            /**si option autre dans lieu stockage
              on récupère la valeur entrée pour l'ajouter à la table lieu_stockage**/
            if ($lieu_stockage == 'autre') {
                //si on est en mode démo, on empêche l'ajout
            ...
            if($_POST['conso_partir'] == '') $conso_partir = NULL;   
            else $conso_partir = $_POST['conso_partir']."-01-01";
            //année consommation maximum
            if($_POST['annee_conso_avant'] == '') $conso_avant = NULL;
            else $conso_avant = $_POST['annee_conso_avant']."-12-31";
            //commentaires
            if($_POST['commentaires'] == '') $commentaires = NULL; 
            else $commentaires = $_POST['commentaires'];
           ...
           // echo $req_user_vin;*/
            ...
            if($erreur != '') header('location:ajouter.php?err='.$erreur);
            if($erreur_vin != '') header('location:ajouter.php?err2='.$erreur_vin);
        }
    }
}
